...
build: ## Builds the binary
	@CGO_ENABLED=0 go build -a -installsuffix cgo -ldflags="-w -s" -o ./bin/$(name_owner) ./cmd/$(name_owner)/*.go
	@CGO_ENABLED=0 go build -a -installsuffix cgo -ldflags="-w -s" -o ./bin/$(name_auditor) ./cmd/$(name_auditor)/*.go
	@CGO_ENABLED=0 go build -a -installsuffix cgo -ldflags="-w -s" -o ./bin/$(name_issuer) ./cmd/$(name_issuer)/*.go
